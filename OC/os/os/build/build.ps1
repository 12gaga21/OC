# Скрипт сборки операционной системы для Windows (PowerShell)
# Требуется: NASM (локальный в os/tests/tools/nasm/), QEMU (установлен)
...
# Компиляция ядра (пока только kernel.asm)
...
# Добавление ядра в образ (добавляем байты ядра к образу)
