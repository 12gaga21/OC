...
; Цвета: Зеленый текст на черном фоне (Классический терминал/Механикус)
; Язык: Дореформенный русский с церковной стилистикой
...
; Цветовая палитра "Священный Люминофор"
; 0x0A = Зеленый текст на черном фоне (Механикус/Терминал)
; 0x0F = Белый на черном (Особые сообщения)
; 0x0E = Желтый на черном (Предупреждения)
...
vga_init:
    ...
    ; Установка цвета по умолчанию (Зеленый ритуальный терминал)
...
section .data
    default_color db 0x1E  ; Золотой на синем по умолчанию
