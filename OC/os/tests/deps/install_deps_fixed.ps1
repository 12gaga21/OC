...
# Функция для добавления пути в PATH пользователя
function Add-ToUserPath {
    ...
    if ($currentPath -split ';' -notcontains $PathToAdd) {
        ...
        Write-Host "   Добавлен $PathToAdd в PATH пользователя" -ForegroundColor Green
        return $true
    } else {
...
# Поиск NASM в стандартных местах
if (-not $nasmFound) {
    $possiblePaths = @(
        "C:\Program Files\NASM\nasm.exe",
        "C:\Program Files (x86)\NASM\nasm.exe",
        "$env:USERPROFILE\AppData\Local\NASM\nasm.exe"
    )
...
if (-not $nasmFound) {
    ...
    Write-Host "   A. Установить через winget (рекомендуется):" -ForegroundColor White
    Write-Host "      winget install NASM.NASM --accept-package-agreements --accept-source-agreements" -ForegroundColor White
    Write-Host ""
    Write-Host "   B. Скачать с официального сайта:" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "   C. Использовать Chocolatey (требует прав администратора):" -ForegroundColor White
    Write-Host "      choco install nasm -y" -ForegroundColor White
    Write-Host ""
    
    $choice = Read-Host "   Выберите вариант (A/B/C) или нажмите Enter для пропуска"
    switch ($choice.ToUpper()) {
        'A' {
            Write-Host "   Устанавливаю NASM через winget..." -ForegroundColor Yellow
            winget install NASM.NASM --accept-package-agreements --accept-source-agreements
            if ($LASTEXITCODE -eq 0) {
                Write-Host "   ✓ NASM успешно установлен" -ForegroundColor Green
                # Попробуем найти путь
                $nasmPath = "C:\Program Files\NASM\nasm.exe"
                if (Test-Path $nasmPath) {
                    $dir = Split-Path $nasmPath -Parent
                    Add-ToUserPath -PathToAdd $dir
                }
            } else {
                Write-Host "   ✗ Ошибка установки NASM" -ForegroundColor Red
            }
        }
        'B' { Write-Host "   Пожалуйста, установите NASM вручную." -ForegroundColor Yellow }
        'C' {
            if ($isAdmin) {
                Write-Host "   Устанавливаю NASM через Chocolatey..." -ForegroundColor Yellow
                choco install nasm -y
            } else {
                Write-Host "   Требуются права администратора для Chocolatey" -ForegroundColor Red
            }
        }
        default { Write-Host "   Пропускаем установку NASM" -ForegroundColor Yellow }
    }
}
...
# Поиск QEMU в стандартных местах
if (-not $qemuFound) {
    $possiblePaths = @(
    ...
    foreach ($path in $possiblePaths) {
        if (Test-Path $path) {
            $qemuPath = $path
            $qemuFound = $true
            Write-Host "   ✓ QEMU найден по пути: $path" -ForegroundColor Green
            $dir = Split-Path $path -Parent
            Add-ToUserPath -PathToAdd $dir
            break
        }
...
if (-not $qemuFound) {
    ...
    Write-Host "   A. Установить через winget:" -ForegroundColor White
    Write-Host "      winget install SoftwareFreedomConservancy.QEMU --accept-package-agreements --accept-source-agreements" -ForegroundColor White
    Write-Host ""
    Write-Host "   B. Скачать с официального сайта:" -ForegroundColor White
    ...
    switch ($choice.ToUpper()) {
        'A' {
            Write-Host "   Устанавливаю QEMU через winget..." -ForegroundColor Yellow
            winget install SoftwareFreedomConservancy.QEMU --accept-package-agreements --accept-source-agreements
            if ($LASTEXITCODE -eq 0) {
                Write-Host "   ✓ QEMU успешно установлен" -ForegroundColor Green
                # Попробуем найти путь
                $qemuPath = "C:\Program Files\qemu\qemu-system-i386.exe"
                if (Test-Path $qemuPath) {
                    $dir = Split-Path $qemuPath -Parent
                    Add-ToUserPath -PathToAdd $dir
                }
            } else {
                Write-Host "   ✗ Ошибка установки QEMU" -ForegroundColor Red
            }
        }
        'B' { Write-Host "   Пожалуйста, установите QEMU вручную." -ForegroundColor Yellow }
        'C' {
            if ($isAdmin) {
                Write-Host "   Устанавливаю QEMU через Chocolatey..." -ForegroundColor Yellow
                choco install qemu -y
            } else {
                Write-Host "   Требуются права администратора для Chocolatey" -ForegroundColor Red
            }
        }
        default { Write-Host "   Пропускаем установку QEMU" -ForegroundColor Yellow }
    }
}
...
if ($bashFound) {
    if ($bashPath -ne "") {
        ...
        $dir = Split-Path $bashPath -Parent
        Add-ToUserPath -PathToAdd $dir
        Write-Host "   Для использования Git Bash перезапустите терминал или выполните:" -ForegroundColor Cyan
